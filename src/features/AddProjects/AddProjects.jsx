...
function AddProjects() {
  ...
  const [nameValue, setNameValue] = useState(() => getStoredValue("nameValue", ""));
  const handleNameChange = (e) => {
    setNameValue(e.target.value);
    localStorage.setItem("nameValue", e.target.value);
  };

  const [startData, setStartData] = useState(() => getStoredValue("startData", ""));
  const handleStartDataChange = (e) => {
    const value = e.target.value;
    if (/^[0-9/]*$/.test(value)) {   // only numbers and slash
      setStartData(value);
  ...
  const [priority, setPriority] = useState(() => getStoredValue("priority", "medium"));
  ...
  return (
    <div className='add-projects-bar'>
      <div style={{ display: isVisible ? "none" : "flex" }} className="add-project-x">
        <img src={mainImage} alt="Add main image" />
        <button onClick={toggleDisplay}>+ Add Project</button>
      </div>
      <div style={{ display: isVisible ? "flex" : "none" }} className='add-projects-container'>
          ...
          <div className="inputs">
            ...
            <div className='add-priority'>
              ...
              <div className="select-div">
                <select onChange={handlePriorityChange} value={priority} id="projectPriority">
        ...
        <div className="add-project-right">
          <div className="project-right-top">
            ...
            <div className="site-logo-opts">
              <div>
                <h3>Select image</h3>
                <p>Select or upload an avatar for the project (available formats: jpg, png)</p>
              </div>
              <ul>
                ...
                <li onClick={returnConstructionBar}><img src={logo12} alt="" /></li>
              </ul>
            </div>
          ...
          <button className='add-project-bottom' onClick={goToMainPage}>Save Project</button>
        </div>
        <button className='x-btn' onClick={toggleDisplay}>✕</button>
      </div>
      <div style={{ display: constructionVisible ? "flex" : "none" }} className="construction-bar">
        <button onClick={removeConstructionBar} className='x-btn2'>✕</button>
      ...
      <div onClick={removeConstructionBar} style={{ display: constructionVisible ? "flex" : "none" }} className="shadow-bar"></div>
    </div>
  )
}
